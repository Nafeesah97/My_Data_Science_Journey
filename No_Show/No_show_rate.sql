...
CREATE VIEW TotalShows AS
SELECT 
	SUM(CASE WHEN No_show LIKE '%No%' THEN 1 ELSE 0 END) AS "TotalNo_Show",
    SUM(CASE WHEN No_show LIKE '%Yes%' THEN 1 ELSE 0 END) AS "TotalShow"
FROM appointments;
...
-- No Show Rate
SELECT ROUND(
	SUM(CASE WHEN No_show LIKE '%No%' THEN 1 ELSE 0 END) / COUNT(*), 2) * 100 AS NoShowRate
FROM appointments;
...
SELECT Gender, COUNT(*) AS total_appointments, (
	ROUND(SUM(CASE WHEN No_show LIKE '%No%' THEN 1 ELSE 0 END) / COUNT(*), 2)
    ) * 100 AS NoShowRate
FROM appointments
GROUP BY gender;
...
SELECT 
    ...
    ROUND(SUM(
    CASE WHEN No_show LIKE '%No%' THEN 1 ELSE 0 
...
SELECT 'Diabetes' AS `CONDITION`,
	COUNT(*) AS total_patient,
    ROUND(SUM(CASE WHEN No_show LIKE '%No%' THEN 1 ELSE 0 
    END) / COUNT(*), 2) * 100 AS NoShowRate
FROM appointments
WHERE Diabetes = 1
...
SELECT Scholarship,
	COUNT(*) AS total_patient,
    SUM(CASE WHEN No_show LIKE '%No%' THEN 1 ELSE 0 END) / COUNT(*) * 100 AS NoShowRate
FROM appointments
